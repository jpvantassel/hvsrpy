...
"""Performance test for HvsrAzimuthal calculation."""
...
def main():
    ...
    processing_settings = hvsrpy.HvsrAzimuthalProcessingSettings()
    processing_settings.smoothing = dict(
        operator="konno_and_ohmachi",
        bandwidth=40,
        center_frequencies_in_hz=np.geomspace(0.1, 50, 256),
    )
    processing_settings.azimuths_in_degrees = np.arange(0, 180, 15)
...
# 2024 - 06 - 05 : 2.228 s -> Remove sigpropy as dependency.
