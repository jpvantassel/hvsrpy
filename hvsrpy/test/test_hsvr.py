...
class Test(unittest.TestCase):
    ...
    def test_update_peaks(self):
        ...
        self.assertListEqual(myhv.peaks.tolist(), frq[col[:-1]].tolist())
    ...
    def test_mean_std_curve(self):
        ...
        for col in range(amp.shape[1]):
            self.assertEqual(np.exp(np.mean(np.log(amp[:,col]))), mean_curve[col])
            self.assertEqual(np.std(np.log(amp[:,col]), ddof=1), std_curve[col])
